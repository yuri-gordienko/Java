...
// знати один одному, це набо і назви Методів які зрозумілі по назві, що виконують

// E findById(Long id); - у реалізації методу повертаємо конкретний обʼєкт return convertResultSetToEmployee(resultSet);,
// або return null;, - таким чином 1- ішший програміст який користується цим методом, розуміє що обʼєкт обовʼязково повернеться і
