...
public class ProductPDPDto {
    ...
    public ProductPDPDto(Product product, Collection<ProductVariant> productVariants) { // принимаем продукт и варианты
        this.id = product.getId();
        this.productBrand = product.getProductBrand().getBrandType();
        this.name = product.getName();
        this.description = product.getDescription();
        Set<ProductImage> productImages = product.getProductImages();   // достаем картинки
        if (CollectionUtils.isNotEmpty(productImages)) {
            this.images = productImages.stream(). // преобразует стрим одного вида в другой, ссылка на медод
                    map(ProductImage::getImageUrl).
                    collect(Collectors.toSet());    // конвертируем их, терминальный оператор, принял стрим - вернул объект
        }
