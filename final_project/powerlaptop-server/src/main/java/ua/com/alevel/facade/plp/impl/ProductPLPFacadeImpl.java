...
public class ProductPLPFacadeImpl implements ProductPLPFacade {
    ...
    public ProductPLPFacadeImpl(ProductCrudService productCrudService) {    // достаем все продукты
        this.productCrudService = productCrudService;
    }

    @Override
    public Collection<ProductPLPDto> findAll() {
        return productCrudService.findAll().stream().map(ProductPLPDto::new).toList(); // конвертируем в ProductPLPDto
    }
}
