...
export class AuthService {
  ...
  register(username: string, password: string): Observable<any> { // функция принимает вводимые данные
    return this.process(username, password, appSettings.apiOpen + '/auth/authenticate'); // передаем бади
  }

  isLoggedIn(): Observable<boolean> {
    return this._sessionService.fromStorage("token") // вытаскиваем токин из памяти (если ранее залогинены)
      .pipe(
        map(token => {
          return !!token;
        })
